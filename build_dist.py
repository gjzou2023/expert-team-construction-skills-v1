...
def transform_frontmatter(fm, skill_id, body=""):
    ...
    if trigger_str:
        # 截取到120字符以内，确保不截断在词中间（优先句子边界）
        desc_limit = 120
        truncate_at = original_desc.rfind('。', 0, desc_limit)
        if truncate_at == -1 or truncate_at < 60:
            truncate_at = original_desc.rfind('，', 0, desc_limit)
        if truncate_at == -1 or truncate_at < 60:
            truncate_at = min(desc_limit, len(original_desc))
        new_desc = f"{original_desc[:truncate_at]} Use when: 用户说\"{trigger_str}\"等触发词。"
    else:
        new_desc = f"{original_desc[:200]}。"

    # 截断到 250 字符（优先在句子边界截断，避免截断在词中间）
    if len(new_desc) > 250:
        # 尝试在 250 字符内的最后一个完整句子截断
        truncate_pos = new_desc.rfind('。', 0, 250)
        if truncate_pos == -1:
            truncate_pos = new_desc.rfind('，', 0, 250)
        if truncate_pos == -1:
            truncate_pos = new_desc.rfind(' ', 0, 250)
        if truncate_pos > 150:  # 确保截断位置不会太短
            new_desc = new_desc[:truncate_pos + 1] + "..."
        else:
...
def generate_meta_json(skill_id, version="1.0.0"):
...
""" + orchestration_logic
    ...
    for skill_id in SKILL_DIRS:
        ...
        # 拷贝 skill 目录下的数据文件（如 platform_template_registry.json）
        for f in src_dir.iterdir():
            if f.is_file() and f.name not in ("SKILL.md", "config.json"):
...
""" + new_body[len("# 全域专家团总编排器"):]
    ...
    # 2b. 拷贝 skill 目录下的数据文件（如 platform_template_registry.json）到 references/data/
    data_ref_dir = ref_dir / "data"
    os.makedirs(data_ref_dir, exist_ok=True)
    for skill_id in SKILL_DIRS:
        src_dir = SRC / skill_id
        for f in src_dir.iterdir():
            if f.is_file() and f.name not in ("SKILL.md", "config.json"):
                shutil.copy2(f, data_ref_dir / f"{skill_id}__{f.name}")
...
"""
    ...
    # 3b. 拷贝 skill 目录下的数据文件（如 platform_template_registry.json）到 references/data/
    data_ref_dir = ref_dir / "data"
    os.makedirs(data_ref_dir, exist_ok=True)
    for skill_id in SKILL_DIRS:
        src_dir = SRC / skill_id
...
def build_hermes():
    ...
    for skill_id in SKILL_DIRS:
        ...
        if skill_id == "team-orchestrator":
            ...
            orch_version = fm.get('version', '1.0.0')
...
"""
    ...
    # 3. 拷贝 knowledge 文件到 hermes/references/
    knowledge_src = SRC / "knowledge"
    if knowledge_src.exists():
        ref_dir = DIST / "hermes" / "references"
        os.makedirs(ref_dir, exist_ok=True)
        for f in knowledge_src.iterdir():
            shutil.copy2(f, ref_dir / f.name)
        print(f"  📁 knowledge 文件已拷贝到 {ref_dir}")
    else:
        print(f"  ⚠️ 源 knowledge 目录不存在，跳过拷贝")

    # 4. 拷贝 skill 目录下的数据文件（如 platform_template_registry.json）
    for skill_id in SKILL_DIRS:
        src_dir = SRC / skill_id
        for f in src_dir.iterdir():
            if f.is_file() and f.name not in ("SKILL.md", "config.json"):
                # 非标准数据文件，拷贝到对应 skill 分发目录
                dst_file = target / skill_id / f.name
