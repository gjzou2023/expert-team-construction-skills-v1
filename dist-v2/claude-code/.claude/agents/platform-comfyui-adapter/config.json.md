{
  ...
  "few_shot_examples": [
    {
      ...
      "output": {
        "status": "completed",
        "decision": "ComfyUI多模态生成适配器已按A型内容传播场景执行",
  ...
  "knowledge_base_mount_points": [
    {
      "type": "static",
      "path": "file://platform/platform-comfyui-adapter/format-spec",
      "description": "ComfyUI多模态生成适配器格式规范"
    },
    {
      "type": "dynamic",
      "path": "file://runtime/platform-comfyui-adapter/state",
      "description": "运行时状态、用户确认和阶段快照"
    },
    {
      "type": "rag",
      "path": "file://rag/platform-comfyui-adapter/references",
  ...
  "execution_logic": "目标平台=ComfyUI多模态生成适配器时激活。输出格式：预设映射配置 + 接口Schema + 视觉角色SOP扩展 + 降级方案。平台特定规则：交付物→预设映射(小红书封面3x4/公众号封面235x1等)。参数快照必须保存：模型版本+种子值+提示词+参数。",
  "cascading_calls": [
    "core-mental-model-engine"
  ],
  "context_inheritance": [
  ...
  "shared_memory_keys": [
    "platform-comfyui-adapter_output"
  ],
  "protocol": {
