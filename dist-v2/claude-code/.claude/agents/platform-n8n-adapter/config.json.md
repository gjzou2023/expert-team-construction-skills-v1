{
  ...
  "few_shot_examples": [
    {
      ...
      "output": {
        "status": "completed",
        "decision": "n8n工作流引擎适配器已按A型内容传播场景执行",
  ...
  "knowledge_base_mount_points": [
    {
      "type": "static",
      "path": "file://platform/platform-n8n-adapter/format-spec",
      "description": "n8n工作流引擎适配器格式规范"
    },
    {
      "type": "dynamic",
      "path": "file://runtime/platform-n8n-adapter/state",
      "description": "运行时状态、用户确认和阶段快照"
    },
    {
      "type": "rag",
      "path": "file://rag/platform-n8n-adapter/references",
  ...
  "execution_logic": "目标平台=n8n工作流引擎适配器时激活。输出格式：工作流模板JSON + 集成接口规范 + D型深度集成配置。平台特定规则：n8n双重角色判断：'调度器'定时/事件触发→调用AI Agent;'执行引擎'D型核心管道。无实例降级：纯提示词(用户手动触发)。",
  "cascading_calls": [
    "core-mental-model-engine"
  ],
  "context_inheritance": [
