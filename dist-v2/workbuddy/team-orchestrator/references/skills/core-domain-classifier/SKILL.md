...
description: 基于S1需求提取关键词，匹配六种领域类型矩阵(A/B/C/D/E/F)，如有歧义使用ToT多方案比较，输出领域确认卡。v1.4.0行业知识骨架扩展至8个高频行业+子领域扩充 Use when: 用户说"core-domain-classifier、领域分类引擎、L0分类"等触发词。
---
...
> **层级**: L0 | **版本**: 1.4.0 | **ID**: `core-domain-classifier`
...
```json
{
  "domain_type": "A",  // 主类型（E型已改为组合标记，primary_domain+secondary_domains）
  "domain_name": "混合型(A+B)",
...
- **[static]** ile://domain-matrix/classification-rules — 六种领域类型分类矩阵
- **[static]** ile://domain-matrix/compliance-activation-map — 领域类型→合规协议条件激活映射
...
FUNCTION execute_core_domain_classifier(input):
    ...
    // ========== 第3步：混合型处理（E型改为A-F的组合标记，不再是独立类型） ==========
    ...
    IF scores[top_type] > 0 AND scores[second_type] > SCORE_THRESHOLD:
        // 多类型得分均超过阈值，判定为混合型（使用组合标记替代E型）
        candidate_type = top_type  // 主类型
        sub_types = IDENTIFY_SUB_TYPES(scores, SCORE_THRESHOLD)
        ASSERT LENGTH(sub_types) >= 2, "混合型必须包含至少两个子类型"
        // 取所含子类型协议并集
        merged_compliance = {}
        merged_feedback = []
    ...
    // E型已改为A-F组合标记，混合型的domain_name用子类型组合表示
    IF LENGTH(sub_types) >= 2:
