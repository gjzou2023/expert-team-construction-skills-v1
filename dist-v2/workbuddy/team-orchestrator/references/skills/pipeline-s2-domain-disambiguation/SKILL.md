...
description: 调用core-domain-classifier，4维度逐个消解歧义(领域本体/目标用户/交付形式/交付渠道)，输出领域确认卡(支持多标签组合+时序演化)。歧义消解强确认 Use when: 用户说"S2执行、领域确认、分类消歧、标签组合、领域画像"等触发词。
---

# 阶段二：领域分类与消歧

> **层级**: L1 | **版本**: 1.1.0 | **ID**: `pipeline-s2-domain-disambiguation` | **中文名**: 阶段二：领域分类与消歧 | **英文名**: Stage 2: Domain Disambiguation
