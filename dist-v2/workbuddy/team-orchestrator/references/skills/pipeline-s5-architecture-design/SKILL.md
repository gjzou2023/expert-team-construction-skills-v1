...
description: 从交付物倒推角色，MECE三问校验，设计SOP/数据流/反馈回路/冷启动策略，执行5.1-5.7自检。v1.3.0新增角色深度画像与结构化辩论机制。角色数≥4必须Team型 Use when: 用户说"S5执行、架构设计、角色倒推、SOP设计、MECE校验"等触发词。
---

# 阶段五：架构设计

> **层级**: L1 | **版本**: 1.3.0 | **ID**: `pipeline-s5-architecture-design` | **中文名**: 阶段五：架构设计 | **英文名**: Stage 5: Architecture Design
