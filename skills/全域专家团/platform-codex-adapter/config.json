{
  ...
  "version": "1.1.0",
  "description": "生成.codex/agents/*.toml+AGENTS.md+.codex/config.toml。description必须英文,developer_instructions≥200字符,model禁止占位符,sandbox_mode必须明确。",
  "trigger_keywords": [
    "codex",
    "codex",
    "平台适配",
  ...
  "few_shot_examples": [
    {
      ...
      "output": {
        "status": "completed",
        "decision": "Codex CLI平台适配器已按A型内容传播场景执行",
  ...
  "knowledge_base_mount_points": [
    {
      "type": "static",
      "path": "file://platform/platform-codex-adapter/format-spec",
      "description": "Codex CLI平台适配器格式规范"
    },
    {
      "type": "dynamic",
      "path": "file://runtime/platform-codex-adapter/state",
      "description": "运行时状态、用户确认和阶段快照"
    },
    {
      "type": "rag",
      "path": "file://rag/platform-codex-adapter/references",
  ...
  "execution_logic": "目标平台=Codex CLI平台适配器时激活。输出格式：.codex/agents/*.toml + AGENTS.md + .codex/config.toml。平台特定规则：无内置调度能力,需外部crontab;输出shell脚本+crontab配置,用户需自行部署。快速通道TOML规则(简化版)。",
  "cascading_calls": [
    "core-mental-model-engine"
  ],
  "context_inheritance": [
