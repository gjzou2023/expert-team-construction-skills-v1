...
description: 全域专家团构建skills系统统一入口编排器。用户只需触发此skill一次，系统自动按S1需求深潜→S2领域消歧→S3链路拆解→S4交付物锚定→S5架构设计→S6工具链匹配→S7专家包生成→S8平台执行的顺序执行全流程，L0/L2/L3/L4层skill按需自动激活，用户无需感知底层39个子skill的存在。
agent_created: true
